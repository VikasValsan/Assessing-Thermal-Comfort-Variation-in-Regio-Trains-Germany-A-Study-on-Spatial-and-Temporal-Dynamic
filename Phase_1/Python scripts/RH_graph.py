...
def plot_relative_humidity(file_path):
    ...
    # Plotting both Relative Humidity and Relative Humidity @1.10m on the same graph
    fig, ax = plt.subplots(figsize=(10, 6))

    try:
        # Plotting both series with updated labels
        ax.plot(data['Time'], data['Relative Humidity'], label='$rh_{ESK}$', color='yellow')
        ax.plot(data['Time'], data['Relative Humidity @1.10m'], label='$rh_{ST}$', color='red')
...
if file_path:
    plot_relative_humidity(file_path)
else:
