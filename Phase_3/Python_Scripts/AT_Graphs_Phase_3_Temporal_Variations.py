...
columns_to_clean = ['HVAC_OutsideTemp_value', 'Point1_Upperright.t_db_value', 'Point1_Lowerright.t_db_value']
...
# Upper Deck
plt.subplot(2, 1, 1)
plt.plot(filtered_data['Time_HHMM'], filtered_data['Point1_Upperright.t_db_value'], label='Air Temp (Upper)', color='orange')
plt.plot(filtered_data['Time_HHMM'], filtered_data['AirTempThreshold'], label='Temp Threshold', linestyle='--', color='green')
plt.plot(filtered_data['Time_HHMM'], filtered_data['HVAC_OutsideTemp_value'], label='Outside Temp', linestyle='--', color='black')
plt.title('Upper Deck - Air Temperature (July 11, 2024)')
...
# Lower Deck
plt.subplot(2, 1, 2)
plt.plot(filtered_data['Time_HHMM'], filtered_data['Point1_Lowerright.t_db_value'], label='Air Temp (Lower)', color='blue')
plt.plot(filtered_data['Time_HHMM'], filtered_data['AirTempThreshold'], label='Temp Threshold', linestyle='--', color='green')
plt.plot(filtered_data['Time_HHMM'], filtered_data['HVAC_OutsideTemp_value'], label='Outside Temp', linestyle='--', color='black')
plt.title('Lower Deck - Air Temperature (July 11, 2024)')
plt.xlabel('Time (HH:MM)')
plt.ylabel('Temperature (°C)')
plt.xticks(np.arange(0, len(filtered_data['Time_HHMM']), step=60), rotation=45, fontsize=10)
plt.ylim(16, 32)
